...
exports.uploadResumeTex = async (req, res) => {
  try {
    ...
    const baseFolder = "Dynamic folders"; 
...
exports.getProfileViews = async (req, res) => {
  try {
      ...
      // Sort profile views by date
      const sortedProfileViews = candidate.ProfileViews.sort((a, b) => b.viewedAt - a.viewedAt);

      res.status(200).json({
          success: true,
          profileViews: sortedProfileViews,
      });
